...
function detail(drive, callback) {
	async.series({
		...
		if (err)
			return callback(err);
		results.freePer = parseInt(results.available / (results.used + results.available) * 10000) / 100;
		results.usedPer = parseInt(results.used / (results.used + results.available) * 10000) / 100;
